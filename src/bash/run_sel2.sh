...
HPT_ITERATIONS=50 # default 50

# Loop through each dataset variation array entry
for entry in "${DATASETS[@]}"; do
    IFS="|" read -r DF_NAME GENO_SUBPATH PHENO_SUBPATH <<< "$entry"
    
    # Updated target subfolder directory strategy for v2 results outputs
    TARGET_OUTDIR="gebvs_v2/${DF_NAME}"    
    ...
    # Run the updated v2 pipeline script
    python3 ../csgs_pred2.py \
