#!/usr/bin/env python3
"""
Genomic selection regression pipeline for predicting continuous phenotype_gwas.
Runs global hyperparameter tuning once using mean squared error, then applies
optimal parameters across 10x repeated 5-fold cross-validation loops.
"""
...
def get_search_spaces():
    return {
        "RR": (
            Ridge(),
            {
                "alpha": Real(1e-5, 100.0, prior="log-uniform"),
            },
        ),
        "EN": (
            ElasticNet(max_iter=10000),
            {
                "alpha": Real(1e-5, 10.0, prior="log-uniform"),
...
def pearson_corr_func(y_true, y_pred):
    ...
    if len(y_true) < 2:
        return 0.0
    if np.allclose(y_pred, y_pred[0]) or np.allclose(y_true, y_true[0]):
        return 0.0
...
def run_global_tuning(X, y, model_name, n_iter):
    ...
    logger.info(
        f"[GLOBAL_OPTIMAL] {model_name} -> Best CV MSE: {-opt.best_score_:.6f}"
    )
