'''
Example usage with both script 01 for hyperparameter tuning and script 02 for cross-validation
...
#  Hyperparameter Tuning with 100 iterations of bayesian optimization
...
# Define custom scorer using pearson correlation between GEBV and trait (same metric for model evaluation)
...
def tune_model(X, y, model_name, n_iter=100):
    ...
    # internal 5-fold cross-validation for each iteration
    opt = BayesSearchCV(
...
def main():
    ...
    #  Save hyperparameters as json for use in cross-validation
    ...
    #  Save full tuned models for later deployment
